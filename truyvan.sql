...
order by (year(curdate())-year(ngaysinh))
desc;
...
select group_concat(concat(maqg1,'-',maqg2),'') as 'cap dau',group_concat(masan,'')as san,ngaytd
from trandau as a
group by ngaytd
having month(ngaytd)=7;
...
select tenhlv, tenqg, round(avg(year(curdate())-year(ngaysinh)),2) as 'tuoitrung binh' 
from (huanluyenvien h inner join quocgia q on h.maqg=q.maqg) inner join cauthu c on q.maqg=c.maqg group by c.maqg 
having round(avg(year(curdate())-year(ngaysinh)),2) < (select round(avg(year(curdate())-year(ngaysinh)),2)from cauthu c inner join quocgia q on c.maqg=q.maqg where tenqg = 'iceland' group by c.maqg) order by round(avg(year(curdate())-year(ngaysinh)),2)asc;
...
select quoctich,group_concat(tenhlv,'') 'ten huan luyen vien',group_concat(tenqg,'') 'ten quoc gia'
from huanluyenvien as a,quocgia as b
where a.maqg=b.maqg
group by quoctich;

--Đưa ra quốc gia có tuổi trung bình trẻ nhất và già nhất
(select tenqg, round(avg(year(curdate())-year(ngaysinh)),2) as 'tuoitrungbinh'
from cauthu c join quocgia q on c.maqg=q.maqg
group by c.maqg
order by round(avg(year(curdate())-year(ngaysinh)),2) asc limit 1)
union
(select tenqg,round(avg(year(curdate())-year(ngaysinh)),2) as 'tuoi trung binh'
from cauthu c join quocgia q on c.maqg=q.maqg
group by c.maqg
order by round(avg(year(curdate())-year(ngaysinh)),2)desc limit 1);
